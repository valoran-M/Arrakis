...
let translate (instruction : basics_inst) addr line labels =
  ...
  | I (inst, rd, rs1, imm) ->
    let imm = expr_to_int32 imm labels line addr in
...
let loop_data mem _ labels addr (prog : data_line) =
  let open String in
  let set_str  a s = Memory.set_str   mem a s (length s) in
  let set_strz a s = Memory.set_strz  mem a s (length s) in
  let set_byte a v = Memory.set_byte  mem a (char_to_int32 v); addr + 1l in
  let set_word a v = Memory.set_int32 mem a v; addr + 4l in
  match prog with
  ...
  | Data_Word words -> List.fold_left set_word addr words
