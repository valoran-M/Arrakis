...
let rec loop arch history label debug in_channel out_channel syscall =
    ...
    (try
      ...
      | command :: args ->
        Shell.parse_command out_channel arch history command args label
          debug syscall;
      | [] -> history) in
        loop arch history label debug in_channel out_channel syscall
...
let start_server file arch history label debug syscall =
  ...
  ignore (loop arch history label debug inchan outchan syscall);
  close sock;
  unlink file
