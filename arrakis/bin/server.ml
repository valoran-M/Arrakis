...
let rec loop arch label addr_debug line_debug in_channel out_channel syscall =
  ...
  | Some s ->
    (try
      ...
      | [] -> ());
      loop arch label addr_debug line_debug in_channel out_channel syscall
...
let start_server file arch label addr_debug line_debug syscall =
  ...
  loop arch label addr_debug line_debug inchan outchan syscall;
  close sock;
  unlink file
