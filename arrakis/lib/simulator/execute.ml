...
let exec (instruction : Int32.t) (cpu : Cpu.t) memory =
  ...
  | 0b0010011l ->
    ...
    let return = I.execute_arith decode rs1 in
  ...
  | 0b0000011l ->
    let decode = I.decode instruction in
    let rs1 = Regs.get cpu.regs decode.rs1 in
    let return = I.execute_load decode rs1 memory in
    let last_value = Regs.get cpu.regs decode.rd in
    Regs.set cpu.regs decode.rd return;
    next_pc cpu; Change_Register (decode.rd, last_value)
  | 0b1100111l ->
  ...
  (* S type *)
  | 0b0100011l ->
    let decode = S.decode instruction in
    let rs1 = Regs.get cpu.regs decode.rs1 in
    let rs2 = Regs.get cpu.regs decode.rs2 in
    let (length, addr, last_value) = S.execute decode rs1 rs2 memory in
    let change = History.create_write_mem length addr last_value in
    next_pc cpu; change
  (* B type *)
  | 0b1100011l ->
    let decode = B.decode instruction in
    let rs1 = Regs.get cpu.regs decode.rs1 in
    let rs2 = Regs.get cpu.regs decode.rs2 in
    let imm = B.execute decode rs1 rs2 in
    add_pc cpu imm; Change_Nothing
  (* U type *)
  | 0b0110111l ->
    let decode = U.decode instruction in
  ...
  (* J Type *)
  | 0b1101111l ->                                    (* JAL *)
    let decode = J.decode instruction in
    let last_value = get_reg cpu decode.rd in
    set_reg cpu decode.rd (Int32.add (get_pc cpu) 4l);
