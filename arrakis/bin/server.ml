...
let rec loop arch history labels debug in_channel out_channel syscall =
  ...
  | Some s ->
    (try
      ...
      | command :: args ->
        Shell.parse_command out_channel arch history command args labels
          debug syscall;
      | [] -> history) in
        loop arch history labels debug in_channel out_channel syscall
...
let start_server file arch history labels debug syscall =
  ...
  ignore (loop arch history labels debug inchan outchan syscall);
  close sock;
  unlink file
